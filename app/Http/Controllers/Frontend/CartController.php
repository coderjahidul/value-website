...
class CartController extends Controller
{
    ...
    public function storeCart(Request $request)
    {
        ...
        // Check variation stock if variation exists, otherwise check product stock
        $variationStock = $variation ? $this->util->checkProductStock($product_id, $variation_id) : 0;
        if ($variation && $variationStock < $quantity) {
            return response()->json(['success' => false, 'msg' => ' Stock Not Available! Only ' . $variationStock . ' items available.']);
        } elseif (!$variation && $product->stock_quantity < $quantity) {
        ...
        if (isset($cart[$variation_id])) {
            ...
            // Check variation stock if variation exists
            if ($variation) {
                $variationStock = $this->util->checkProductStock($product_id, $variation_id);
                if ($request->is_stock != 0 && $variationStock < $newQty) {
                    return response()->json(['success' => false, 'msg' => ' Stock Not Available! Only ' . $variationStock . ' items available.']);
                }
            } elseif ($request->is_stock != 0 && $product->stock_quantity < $newQty) {
                return response()->json(['success' => false, 'msg' => ' Stock Not Available!']);
            }
