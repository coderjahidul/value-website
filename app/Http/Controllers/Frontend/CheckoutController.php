    ...
    public function store(Request $request){
        ...
        try {
            ...
            if(!$order){
                ...
                // Restore stock from old order details before deleting them
                $oldDetails = $order->details()->get();
                foreach ($oldDetails as $oldDetail) {
                    if($oldDetail->is_stock != 0) {
                        $this->util->increaseProductStock($oldDetail->product_id, $oldDetail->variation_id, $oldDetail->quantity);
                    }
                }
