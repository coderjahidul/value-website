...
class LandingPageController extends Controller
{
    ...
    {
        // Handle array fields - convert to single values if needed
        if (is_array($request->product_id)) {
            $pIds = $request->product_id;
            $request->merge(['product_id' => end($pIds)]);
        }
        ...
        // Handle des1 as array - convert to JSON
        if ($request->has('des1') && is_array($request->des1)) {
            // Filter out empty values
            $des1Array = array_filter($request->des1, function($value) {
                return !empty(trim($value));
            });
            // Convert to JSON if not empty
            if (!empty($des1Array)) {
                $data['des1'] = json_encode(array_values($des1Array));
            } else {
                $data['des1'] = null;
            }
        }

        // Filter out array values from $data to prevent "Array to string conversion" error
        // Also explicitly only include the fields we want to prevent any unexpected fields
        $allowedFields = [
            'title1', 'video_url', 'landing_bg', 'feature', 'review_top_text',
            'old_price', 'new_price', 'phone', 'pay_text', 'product_id',
            'regular_price_text', 'offer_price_text', 'call_text',
            'left_side_title', 'left_side_desc', 'right_side_title', 'right_side_desc',
            'top_heading_text', 'left_product_details', 'image', 'right_product_image', 'des1'
        ];
        
        $data = array_intersect_key($data, array_flip($allowedFields));
        
        // Remove any array values (safety check) - but keep des1 as it's already converted to JSON string
        $data = array_filter($data, function($value, $key) {
            return $key === 'des1' || !is_array($value);
        }, ARRAY_FILTER_USE_BOTH);

        // Set page_type if not provided (default to '1' to match index filter)
