...
@php
  ...
  Log::info('Product data from resources/views/frontend/products/show.blade.php: '. json_encode($data));
  Log::info('Single product sizes data from resources/views/frontend/products/show.blade.php: '. json_encode($singleProduct->sizes));
  Log::info('Single product variations data from resources/views/frontend/products/show.blade.php: '. json_encode($singleProduct->variations));
...
@push('css')
  <style>
    ...
    .nav-tabs .nav-link.active {
      color: {{ $info->primary_color }} !important;
      background: {{ $info->gradient_code }} !important;
      border-color: rgba(29, 95, 171, 1);
    }
              ...
              <div class="row mx_0">
                    ...
                    <div class="single-product-thumbnail product-large-thumbnail-3 img-section axil-product">
                      <div class="thumbnail h-100 overflow-hidden">
                        <a href="{{ getImage('products', $singleProduct->image) }}" class="popup-zoom">
                          <img src="{{ getImage('products', $singleProduct->image) }}"
                               alt="{{ $singleProduct->name }} Images">
                        </a>
                      </div>
                      {{-- Display Single product Images --}}
                ...
                <div class="col-lg-2 order-lg-1 px-lg-0">
                  ...
                  <div class="product-small-thumb-3 small-thumb-wrapper">
                    ...
                    {{-- Display Small Thumbnails Images --}}
                    @foreach ($singleProduct->images as $im)
                      <div class="small-thumb-img mt-2">
                ...
                <div class="product">
                  <div class="product-cart">
                    ...
                    <p class="details-price">
                     ...
                     {{-- Display Stock --}}
                     <div class="mb-2 stock-display-wrapper">
                       @if (isset($singleProduct->variations) && $singleProduct->variations->count() > 0)
                         <span class="badge bg-secondary text-light stock-badge">Select a variation to see stock</span>
                       @else
                         {{-- Display Stock Quantity --}}
                         <span
                               class="badge bg-{{ $singleProduct->stock_quantity && $singleProduct->stock_quantity > 0 ? 'success' : 'danger' }} text-light">
                           {{ $singleProduct->stock_quantity && $singleProduct->stock_quantity > 0 ? $singleProduct->stock_quantity : '0' }}
                           Items left
                         </span>
                       @endif
                     </div>
                      ...
                      <div id="sizess" class="mt-3">
                        ...
                        <div class="d-flex flex-wrap gap-2">
                          @foreach ($singleProduct->variations as $var)
                            @php
                              Log::info("Variation stocks: " . $var->stocks);
                              $active = !empty($var->is_default) && (int) $var->is_default === 1 ? ' active' : '';
                              $stockQty = $var->stocks->where('product_id', $singleProduct->id)->sum('quantity') ?? 0;
                            @endphp
                            <div class="size rounded{{ $active }} border p-2" data-vid="{{ $var->id }}"
                                 data-varprice="{{ $var->price }}" data-disprice="{{ $var->discount_price ?? '' }}"
                                 data-size="{{ e($var->size_label) }}" data-color="{{ e($var->color_label) }}"
                                 data-proid="{{ $var->product_id }}" data-stock="{{ $stockQty }}"
                        ...
                        <span class="size_name d-block text-success fw-bold mt-2"></span>
                      </div>
                    @endif
                    {{-- ===== /Variation Block ===== --}}
                    ...
                    <form action="{{ route('front.carts.storeCart') }}" id="cart_submit" method="POST">
                      ...
                      <input type="hidden" id="price_val" name="price_val"
                             value="{{ $singleProduct->after_discount > 0 ? $singleProduct->after_discount : $singleProduct->sell_price }}">
                      <input type="hidden" id="price_val1" name="price_val1"
                             value="{{ $singleProduct->after_discount > 0 ? $singleProduct->after_discount : $singleProduct->sell_price }}">
                      ...
                      <div class="mt-3">
                        ...
                        <div class="d-flex single_product col-sm-12">
                          <button type="submit" class="btn add_cart_btn animated_text px-4" name="action_type"
                                  value="cart" style="font-family: 'Hind Siliguri', sans-serif">
                            কার্টে যোগ করুন
                          </button>
                          <button type="submit" class="btn order_now_btn animated_text order_now_btn_m main-bg px-4"
                                  style="font-family: 'Hind Siliguri', sans-serif" name="action_type" value="order">
                            অর্ডার করুন
                          </button>
                        </div>
                      </div>
                      ...
                      <div class="mt-md-2 mt-2">
                        <div class="mt-2 shadow">
                          <a href="tel:{{ $info->supp_num1 }}"
                             class="btn btn-primary main-bg d-block fs-3 text-light fw-bolder py-3"
                             style="font-family: 'Hind Siliguri', sans-serif;">
                            কল করুন <i class="fas fa-phone-volume"></i> {{ $info->supp_num1 }}
                          </a>
                        </div>
                        <div class="mt-2 shadow">
                          <a href="https://wa.me/+88{{ $whatsappNumber }}?text={{ $encodedMessage }}" target="_blank"
                             class="btn btn-success d-block fs-3 text-light fw-bolder py-3">
                      ...
                      <div class="mt-md-2 mt-2">
                        <table class="table-bordered border-1 border-dark table"
                               style="font-family: 'Hind Siliguri', sans-serif;">
                          <tbody>
                            <tr>
                            ...
                            @foreach ($charges as $charge)
                              <tr>
                                <td class="border-dark border">{{ $charge->title }}</td>
                                <td class="border-dark border text-end">৳ {{ number_format($charge->amount, 0) }}</td>
                              </tr>
                            @endforeach
                          </tbody>
                        </table>
                          ...
                          <div class="wrapper">
                            <div class="master">
                              ...
                              <div class="feedback-tags">
                                ...
                                <div class="tags-box">
                                  <form action="{{ route('front.product-reviews.store') }}" method="POST"
                                        id="ajax_form2" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" name="product_id" value="{{ $singleProduct->id }}" />
...
@push('js')
  <script>
    $(document).ready(function() {
      ...
      function viewContent() {
        ...
        let sell_price =
          {{ isset($singleProduct->after_discount) && $singleProduct->after_discount > 0 ? $singleProduct->after_discount : $singleProduct->sell_price }};
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      plus.addEventListener('click', () => {
        ...
        let maxQty = parseInt(input.getAttribute('max')) || 999999;
        if (value < maxQty) {
          input.value = value + 1;
        }
      });
  ...
  <script type="text/javascript">
    ...
    $(document).on('submit', 'form#cart_submit', function(e) {
      ...
      let form = $(this);
      // Force variation_id from stored selection (set when user clicks a variation)
      var vid = form.data('selectedVariationId');
      if (vid == null || vid === '') {
        var $sizess = form.closest('.product-cart').find('#sizess');
        var $active = $sizess.find('.size.active');
        if ($active.length) vid = $active.attr('data-vid');
      }
      if (vid != null && vid !== '') {
  ...
  <script type="text/javascript">
    ...
    const BASE_FINAL_PRICE =
      {{ (float) ($singleProduct->after_discount > 0 ? $singleProduct->after_discount : $singleProduct->sell_price) }};
    ...
    $('#sizess').on('click', '.size', function() {
      ...
      let value = $(this).attr('value') ||
        ($(this).data('size') + (($(this).data('color') && $(this).data('color') !== 'Default') ? ('-' + $(this).data(
          'color')) : ''));
      ...
      if (stockQty > 0) {
        $('.stock-badge').remove();
        $('.stock-display-wrapper').html('<span class="badge bg-success text-light stock-badge">' + stockQty + ' Items left</span>');
      } else {
        $('.stock-badge').remove();
        $('.stock-display-wrapper').html('<span class="badge bg-danger text-light stock-badge">Out of Stock</span>');
      }

      // update quantity input max
      $('input[name="quantity"]').attr('max', stockQty);
      if (parseInt($('input[name="quantity"]').val()) > stockQty && stockQty > 0) {
        $('input[name="quantity"]').val(stockQty);
      }

      // update hidden fields
      $('#price_val').val(finalPrice);
      if ($('#price_val1').length) {
        $('#price_val1').val(finalPrice);
      }
      $("#size_value").val(value);
      if ($("#size_value1").length) {
        $("#size_value1").val(value);
      }

      // set variation info to form (read from DOM to avoid jQuery data cache issues)
      var vid = $(this).attr('data-vid');
      var vname = value; // display title/size-color
      var $form = $('#cart_submit');
      $form.find('input[name="variation_id"]').val(vid);
      $form.find('input[name="variant_name"]').val(vname);
      $form.data('selectedVariationId', vid);
