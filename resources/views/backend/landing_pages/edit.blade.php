            ...
            <div class="row">
              <div class="col-lg-12">
                <div class="mb-3">
                  ...
                  <input type="text" value="{{ $item->title1 }}" name="title1" class="form-control"
                         placeholder="Title">
              ...
              <div class="col-lg-12">
                <div class="mb-3">
                  ...
                  <input type="text" name="regular_price_text" value="{{ $item->regular_price_text }}"
              ...
              <div class="col-lg-12">
                <div class="mb-3">
                  <label class="form-label">Description (des1)</label>
                  <div id="des1-container">
                    @php
                      $des1Items = [];
                      if ($item->des1) {
                        $decoded = json_decode($item->des1, true);
                        $des1Items = is_array($decoded) ? $decoded : [$item->des1];
                      }
                      if (empty($des1Items)) {
                        $des1Items = [''];
                      }
                    @endphp
                    @foreach($des1Items as $index => $des1Item)
                      <div class="des1-item mb-2" data-index="{{ $index }}">
                        <div class="input-group">
                          <textarea class="form-control des1-field" id="des1_edit_{{ $index }}" name="des1[]" rows="3" placeholder="Enter description">{{ $des1Item }}</textarea>
                          <button type="button" class="btn btn-danger remove-des1" {{ count($des1Items) == 1 ? 'style="display:none;"' : '' }}>
                  ...
                  <button type="button" class="btn btn-success mt-2" id="add-des1">
                    <i class="mdi mdi-plus"></i> Add New Description
                  </button>
                </div>
              </div>
              ...
              <div class="col-lg-12">
                <div class="mb-3">
                  ...
                  <input type="text" name="right_side_title" value="{{ $item->right_side_title }}"
                         class="form-control" placeholder="Title">
                </div>
              </div>
              ...
              <div class="col-lg-12">
                <div class="mb-3">
                  ...
                  <input type="text" name="top_heading_text" value="{{ $item->top_heading_text }}"
                         class="form-control" placeholder="Title">
                </div>
              </div>
              ...
              <div class="col-lg-12 mb-3">
                <div class="d-flex">
                  @foreach ($item->images as $key => $image)
                    <div class="img-box">
                      <a href="{{ route('admin.delete_slider', [$image->id]) }}" class=""
              ...
              <div class="col-lg-12 mb-3">
                <div class="mb-3">
                  ...
                  <input type="text" name="review_top_text" value="{{ $item->review_top_text }}"
                         class="form-control" placeholder="Review Text">
                </div>
              </div>
              ...
              <div class="col-lg-12 mb-3">
                <div class="d-flex">
                  @foreach ($review_images as $key => $review_image)
                    <div class="img-box">
                      <a href="{{ route('admin.delete_review', [$review_image->id]) }}" class=""
                         onclick="return confirm(' you want to delete?');">&times;</a>
              ...
              <div class="product_table" id="data">
                <table class="table-centered table-nowrap mb-0 table" id="product_table">
                  ...
                  <tbody id="data">
                    @if ($single_product)
                      <tr>
                        <td><img src="{{ getImage('products', $single_product->image) }}" height="50"
                               width="50" /></td>
                        <td>{{ $single_product->name }}</td>
                        <td>{{ $single_product->variation->size->title ?? 'N/A' }}</td>
                        <td>{{ $single_product->variation->color->name ?? 'N/A' }}</td>
...
@push('js')
  ...
  <script>
    ...
    // Handle des1 add/remove functionality
    let des1Index = {{ count($des1Items ?? []) }};
    
    $('#add-des1').on('click', function() {
      const fieldId = 'des1_edit_' + des1Index;
      const newField = `
        <div class="des1-item mb-2" data-index="${des1Index}">
          <div class="input-group">
            <textarea class="form-control des1-field" id="${fieldId}" name="des1[]" rows="3" placeholder="Enter description"></textarea>
            <button type="button" class="btn btn-danger remove-des1">
              <i class="mdi mdi-delete"></i> Remove
            </button>
          </div>
        </div>
      `;
      $('#des1-container').append(newField);
      des1Index++;
    ...
    $(document).on('click', '.remove-des1', function() {
      $(this).closest('.des1-item').remove();
      updateRemoveButtons();
    });

    function updateRemoveButtons() {
      const items = $('#des1-container .des1-item');
      if (items.length <= 1) {
        $('.remove-des1').hide();
      } else {
        $('.remove-des1').show();
