      ...
      <div class="card-body">
        ...
        <form method="POST" action="{{ route('admin.products.update',[$item->id])}}" id="ajax_form" enctype="multipart/form-data">
          ...
          <div class="row g-3 mb-2">
            ...
            <div id="stock_qty" class="col-lg-3 col-md-6 {{ $item->type=='variable' ? 'd-none' : '' }}">
              ...
              <table class="table table-centered table-nowrap table-bordered text-center align-middle responsive-table">
                ...
                @forelse($item->variations as $v)
                  <tr>
                    ...
                    <td data-label="Qty">
                      <input class="quantity form-control" type="number" step="any" value="{{ $v->stocks->where('product_id', $item->id)->sum('quantity') ?? 0 }}" name="quantity[]" placeholder="Qty">
                    </td>
...
$(function(){
  ...
  function toggleStock(){
    const isVariable = $('#prod_type').val()==='variable';
    if(isVariable){
      $('#stock_qty').hide();
    }else{
